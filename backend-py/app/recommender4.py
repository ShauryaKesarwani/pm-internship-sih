...
class Recommender:
    ...
    def semantic_score(self, candidate, job):
        """
        Compute per-skill / per-requirement similarity.
        Returns 0-1 semantic score.
        """
        if not candidate.skills or not job.get("requirements"):
            return 0.0

        # Embed candidate skills
        skill_embeddings = self.embed(candidate.skills)  # shape: [num_skills, dim]
        # Embed job requirements
        job_req_embeddings = self.embed(job.get("requirements", []))  # shape: [num_reqs, dim]

        # Compute cosine similarity matrix
        cos_sim_matrix = util.cos_sim(skill_embeddings, job_req_embeddings).numpy()  # shape: [num_skills, num_reqs]

        # For each skill, take max similarity with any job requirement
        max_per_skill = cos_sim_matrix.max(axis=1)  # shape: [num_skills]

        # Average to get overall semantic score
        semantic_score = float(max_per_skill.mean())
